...
{
    public class CreateAccountsPayablePaymentDtoValidator : AbstractValidator<CreateAccountsPayablePaymentDto>
    {
        public CreateAccountsPayablePaymentDtoValidator()
        {
            RuleFor(x => x.AccountsPayableId)
                .NotEmpty().WithMessage("El ID de la cuenta por pagar es requerido");
